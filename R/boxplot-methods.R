...
setMethod("boxplot", signature(x = "ReferenceBias"), function(x, strand="*",
	...
	#add axis labels
	if(labels.axis){
		mtext("Nr. Of Heterozygots Used",side=3, padj=-3, cex=0.9, font=2)
		mtext("Reference Allele Frequency", side=2,padj=-3, cex=0.9, font=2)
		mtext("Samples",side=1,padj=3, cex=0.9, font=2)
	}
