#'@include initialize-methods.R
NULL

#' transform to array
#' 
#' transform ranges to compatible ASEset array
#' 
#' @name txToArray
#' @rdname txToArray
#' @aliases txToArray 
#' txToArray,ASEset-method 
#' @docType methods
#' @param pathGFF path to gff file
#' @param verbose makes function more talkative
#' @param ... arguments to pass on
#' @author Jesper R. Gadin
#' @keywords array
#' @examples
#'
#' #generate example ASEset
#' snps <- 1000
#' samples <-5
#' ar <-array(rep(unlist(lapply(1:snps,
#' 			function(x){(sample(c(TRUE,FALSE,TRUE,FALSE), size = 4))})), samples), 
#' 			dim=c(4,snps,samples))
#' ar2 <- array(sample(50:300, 4*snps*samples,replace=TRUE), dim=c(4,snps,samples))
#' ar2[ar] <- 0
#' ar2 <- aperm(ar2, c(2, 3, 1))
#' dimnames(ar2) <- list(paste("snp",1:snps,sep=""),paste("sample",1:samples,sep=""),
#'							c("A","C","G","T"))
#' 
#' #make example rowRanges for the corresponding information
#' gr <- GRanges(
#' 	seqnames = Rle(sample(paste("chr", 1:3,sep=""),snps, replace=TRUE)),
#' 	ranges = IRanges(1:snps, width = 1, names= paste("snp",1:snps,sep="")),
#' 	strand="*"
#' )
#' #make ASEset from array
#' a <- ASEsetFromArrays(gr, countsUnknown=ar2)
#' 
#' # gene annotation
#' geneGR <- GRanges(
#' 	seqnames = Rle(sample(paste("chr", 1:3,sep=""),snps, replace=TRUE)),
#' 	ranges = IRanges(start = c(1,302,602), end = c(300, 600, 900), names= paste("gene",1:3,sep="")),
#' 	strand=c("+","-","+")
#' )
#' 
#' ### JAG VILL INTE HA ARRAY, bättre med matrix
#' 
#'  
NULL

#' @rdname txToArray
#' @export
setGeneric("txToArray", function(pathBam, ... 
	){
    standardGeneric("txToArray")
})

#' @rdname txToArray
#' @export
setMethod("txToArray", signature(pathBam = "character"),
function(pathBam,pathVcf,pathGFF=NULL, verbose){

	#summarize counts
	
	#detectAI

	
})


