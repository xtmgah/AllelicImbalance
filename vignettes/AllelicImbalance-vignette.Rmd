...
the AllelicImbalance package has now functions that can be integrate that
environment. Another good reason for grid graphics is the Gviz package, which
uses tracks to visualize genomic regions in a smart way. Below are examples
on corresponging bar- and locationplots using the grid and Gviz
evironment.

##lbarplots
The "l" in lbarplot comes from "lattice barplot". lattice is an R-package using
grid graphics.
...
##Top allele criteria
The barplot with type='fraction' has the options to be visualized having 
three different criterias for which allele or phased is the top allele.
For clarity, what is defined as the top fraction is the fraction starting
from 1 and going downwards, as can be seen in the figure below. In this case
the top fraction only covers 50%, but in general it can be anywhere between
100% to 0%. 
...
### The default maxcount criteria
The default maxcount criteria is based on the rank for the total number of
reads covering each allele, i.e. the allele covered the most will be on the top.

### Reference allele information
In previous sections it has been described how to import and store reference data
in ASEset objects, here there will be an example of how that information can be
used in a plot

```{r echo=TRUE, eval=TRUE, message=FALSE}
data(ASEset)
mcols(ASEset)[,"alt"] <- inferAltAllele(ASEset)
barplot(ASEset[1], type="fraction", strand="+", top.allele.criteria="ref")
```

###Dealing with phased data in plots
In previous sections it has been described how to import and store phased data
in ASEset objects, here there will be an example of how that information can be
used in a plot

```{r echo=TRUE, eval=TRUE, message=FALSE}
phase(ASEset) <- phase.mat 
barplot(ASEset[1], type="fraction", strand="+", top.allele.criteria="phase")
```
